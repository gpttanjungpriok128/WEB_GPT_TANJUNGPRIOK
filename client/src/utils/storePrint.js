...
function buildOfflineReceiptMarkup(order, { logoUrl, qrValue } = {}) {
  ...
  const itemRows = items.length > 0
    ? items.map((item) => (
      `<tr><td>${escapeHtml(item.productName || "-")}</td><td class="muted">(${escapeHtml(item.size || "-")} x${Number(item.quantity) || 0})</td></tr>`
    )).join("")
    : `<tr><td colspan="2" class="muted">Tidak ada item.</td></tr>`;
  ...
  return `<div class="label"><div class="header"><div class="brand">${logoUrl ? `<img src="${logoUrl}" alt="GTshirt" class="logo" />` : ""}<div><div class="brand-name">GTshirtwear</div><div class="muted">Struk Offline Store</div></div></div>${qrUrl ? `<img src="${qrUrl}" alt="QR ${escapeHtml(order?.orderCode)}" class="qr" />` : ""}</div><div class="muted">Tanggal: ${escapeHtml(formatDateTime(order?.createdAt))}</div><div class="section"><div class="label-title">Pelanggan</div><div class="value">${escapeHtml(order?.customerName || "-")}</div><div class="muted">WA: ${escapeHtml(order?.customerPhone || "-")}</div><div class="muted">Kasir: ${escapeHtml(order?.cashierName || "-")}</div></div><div class="section"><div class="label-title">Transaksi</div><div class="value">${escapeHtml(order?.paymentMethod || "-")}</div><div class="muted">Dibayar: ${escapeHtml(formatRupiah(order?.amountPaid))}</div><div class="muted">Kembalian: ${escapeHtml(formatRupiah(order?.changeAmount))}</div></div><div class="section"><div class="label-title">Item</div><table>${itemRows}</table></div><div class="section"><div class="label-title">Ringkasan</div><div class="summary-row"><span>Subtotal</span><span>${escapeHtml(formatRupiah(order?.subtotal))}</span></div><div class="summary-row strong"><span>Total</span><span>${escapeHtml(formatRupiah(order?.totalAmount))}</span></div></div>${order?.notes ? `<div class="section"><div class="label-title">Catatan</div><div class="muted">${escapeHtml(order.notes)}</div></div>` : ""}${order?.reversedAt ? `<div class="section"><div class="label-title">${reversalTitle}</div><div class="muted">${escapeHtml(formatDateTime(order.reversedAt))}</div>${order?.reversalReason ? `<div class="muted">Alasan: ${escapeHtml(order.reversalReason)}</div>` : ""}</div>` : ""}<div class="footer"><div>Total Item: ${totalItems}</div><div>${escapeHtml(formatRupiah(order?.totalAmount))}</div></div><div class="order-code">Struk: ${escapeHtml(order?.orderCode || "-")}</div></div>`;
}
...
export function buildStoreOrderPrintDocumentFromAssets(assets = []) {
  ...
  return `<!doctype html><html lang="id"><head><meta charset="utf-8" /><title>Resi Pesanan</title><style>@page{size:${isA4 ? "A4" : "A6"};margin:${isA4 ? "8mm" : "0"}}*{box-sizing:border-box}html,body{width:${isA4 ? "210mm" : "105mm"};min-height:${isA4 ? "297mm" : "148mm"}}body{font-family:"Arial",sans-serif;color:#0f172a;margin:0;padding:0;background:#fff}.sheet{${isA4 ? "display:grid;grid-template-columns:repeat(2,1fr);gap:6mm;align-content:start" : ""}}.label{width:${isA4 ? "94mm" : "105mm"};min-height:${isA4 ? "137mm" : "148mm"};padding:${isA4 ? "6mm" : "8mm"};border:${isA4 ? "1px dashed #0f766e" : "0"};border-radius:${isA4 ? "10px" : "0"};margin:0;break-inside:avoid;page-break-inside:avoid}.header{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:8px}.brand{display:flex;align-items:center;gap:10px}.logo{width:42px;height:42px;object-fit:cover;border-radius:10px;border:1px solid #e2e8f0}.brand-name{font-weight:800;font-size:16px;letter-spacing:0.04em;color:#0f766e}.qr{width:84px;height:84px}.section{margin-top:10px;padding-top:10px;border-top:1px solid #e2e8f0}.label-title{font-size:11px;letter-spacing:0.18em;font-weight:700;color:#64748b;text-transform:uppercase}.value{font-size:13px;font-weight:600;margin-top:4px}.muted{color:#64748b;font-size:11px}.summary-row{display:flex;justify-content:space-between;gap:12px;margin-top:4px;font-size:12px}table{width:100%;border-collapse:collapse;margin-top:6px}td{padding:4px 0;font-size:12px;vertical-align:top}.footer{margin-top:10px;display:flex;justify-content:space-between;font-size:12px;font-weight:700}.order-code{margin-top:8px;font-size:12px;font-weight:700;color:#0f766e}</style></head><body>${isA4 ? `<div class="sheet">${markup}</div>` : markup}</body></html>`;
}
